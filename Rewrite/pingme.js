...
if (typeof $request !== 'undefined' && $request) {
  ...
  for (let i = 0; i < accounts.length; i++) {
    if (getStaticStr(accounts[i].paramsRaw) === curStatic) {
      foundIndex = i;
      break;
    }
  }
  ...
  if (foundIndex !== -1) {
    accounts[foundIndex] = capture;
    notifyDone('✅ 参数更新成功', `已更新账号 ${foundIndex + 1} 请求头+参数`);
  } else {
    accounts.push(capture);
    notifyDone('✅ 参数抓取成功', `已新增账号 ${accounts.length} 请求头+参数`);
  }

  $prefs.setValueForKey(JSON.stringify(accounts), ckKey);
  console.log(`【${scriptName}】capture accounts:\n${JSON.stringify(accounts, null, 2)}`);
  $done({});
} else {
  const raw = $prefs.valueForKey(ckKey);
  if (!raw) {
    ...
    function runAccount(capture, index) {
        ...
        function doVideoLoop(count) {
          ...
          function next() {
            ...
            return new Promise(res => {
              setTimeout(() => {
                i++;
                fetchApi('videoBonus').then(resData => {
                  try {
                    const d = JSON.parse(resData.body);
                    if (d.retcode === 0) {
